...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            if !ArmagnacWineInstaller.isArmagnacWineInstalled() {
                showSetup = true
            }
            let task = Task.detached {
                return await ArmagnacWineInstaller.shouldUpdateArmagnacWine()
            }
            let updateInfo = await task.value
            if updateInfo.0 {
                let alert = NSAlert()
                alert.messageText = String(localized: "update.armagnacwine.title")
                alert.informativeText = String(format: String(localized: "update.armagnacwine.description"),
                                               String(ArmagnacWineInstaller.armagnacWineVersion()
                                                      ?? SemanticVersion(0, 0, 0)),
                                               String(updateInfo.1))
                alert.alertStyle = .warning
                alert.addButton(withTitle: String(localized: "update.armagnacwine.update"))
