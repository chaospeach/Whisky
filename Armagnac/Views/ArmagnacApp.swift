        ...
        WindowGroup {
            ContentView(showSetup: $showSetup)
                ...
                .onAppear {
                    ...
                    Task.detached {
                        await ArmagnacApp.deleteOldLogs()
                    }
                }
        }
...
//            }
            ...
            CommandGroup(after: .importExport) {
                Button("open.logs") {
                    ArmagnacApp.openLogsFolder()
                }
                .keyboardShortcut("L", modifiers: [.command])
                Button("kill.bottles") {
                    ArmagnacApp.killBottles()
                }
                .keyboardShortcut("K", modifiers: [.command, .shift])
                Button("wine.clearShaderCaches") {
                    ArmagnacApp.killBottles() // Better not make things more complicated for ourselves
                    ArmagnacApp.wipeShaderCaches()
                }
            }
            CommandGroup(replacing: .help) {
                Button("help.website") {
                    if let url = URL(string: "https://github.com/chaospeach/Armagnac") {
                        openURL(url)
                    }
                }
                Button("help.github") {
                ...
                Button("help.discord") {
                    if let url = URL(string: "https://discord.gg/") {
                        openURL(url)
                    }
                }
            }
